...
def generate_image():
    ...
    image_temp_name="Circle_area_val"
...
def getSolution( answer ):
    ...
    if(given_term=="diameter"):
        d=2*r
        return f"""\n--------------------------------------SOLUTION-------------------------------------------
                The area of circle is given as {latex(area)}.
                Formula of Area = {latex(pi+superscript('r',2))}.
                         {latex(superscript('r',2))}={latex(to_frac(area,pi))}
                                ={latex(r2)}
...
def getQuestion():
    return f"Find the value of {latex('x')} if the area of the circle is {latex(area)}?"

def changequestion():
    global given_term,a,area,multiple_q
