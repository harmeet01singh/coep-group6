...
def generate_img():
	...
	y = total_dist - x
	draw.text([margin + total_dist * 2 - y - 5, 155], "x", font=ImageFont.truetype("arial.ttf", size=25), fill="black")
	draw.text([margin + total_dist - 10, 205], "?", font=ImageFont.truetype("arial.ttf", size=25), fill="black")
	# Places name
	...
	# Display Image
	# img.show()
	image_name = f"Positive_Distance_Variable_{i}.png"
	img.save(image_name, format="PNG")
	return image_name


def getQuestion():
	return f"Find total distance between {a} and {c} in terms of x ?"


def getCorrOption():
	return f"{latex(str(x) + ' + x')}"


def getWrongOptions():
	options = [f"{latex(str(x) + ' - x')}", f"{latex('x - ' + str(x))}", f"{latex('-(x + ' + str(x) + ')')}"]
	random.shuffle(options)
	return options


def getSolution():
	return f"\n--------------------------------------SOLUTION-------------------------------------------\n=> distance between {a} and {c} = distance between {a} and {b} + distance between {b} and {c} \n=> distance between {a} and {c} = {latex(str(x) + ' + x')} \nHence the correct answer is {latex(str(x) + ' + x')}."
...
putInCsv(
	...
	Filename="Positive_Distance_Variable.py"
)
