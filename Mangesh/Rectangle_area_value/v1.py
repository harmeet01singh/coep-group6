...
def solution(correctAns, correctOption,l1,b1):
    return f'''\n--------------------------------------SOLUTION------------------------------------------- => Area of Rectangle = length * breadth => x = {latex(int(correctAns/l1))} => ThereFore Option {latex(correctOption)} is right selection'''

def getQuestion(area):
    return f"Q) What will be the value of x for given rectangle, here given area is {latex(area)} ?"
